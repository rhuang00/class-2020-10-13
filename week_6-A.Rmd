...
p_draws <- tibble(replicate = 1:1000) %>%
  mutate(samp = map(replicate, ~ sample_n(urn, size = 50))) %>% 
  mutate(red = map_int(samp, ~sum(.$color == "red"))) %>% 
  mutate(blue = map_int(samp, ~sum(.$color == "blue"))) %>% 
  mutate(red_win = ifelse(red > blue, TRUE, FALSE)) %>% 
  summarise(probs = mean(red_win))
